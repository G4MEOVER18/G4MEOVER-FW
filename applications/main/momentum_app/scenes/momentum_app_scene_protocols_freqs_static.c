...
void g4meover_app_scene_protocols_freqs_static_var_item_list_callback(
    void* context,
    uint32_t index) {
...
static void g4meover_app_scene_protocols_freqs_static_frequency_changed(VariableItem* item) {
...
void g4meover_app_scene_protocols_freqs_static_on_enter(void* context) {
    G4MEOVERApp* app = context;
    ...
    item = variable_item_list_add(
        ...
        g4meover_app_scene_protocols_freqs_static_frequency_changed,
        app);
    ...
    variable_item_list_set_enter_callback(
        var_item_list, g4meover_app_scene_protocols_freqs_static_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(app->scene_manager, G4MEOVERAppSceneProtocolsFreqsStatic));

    view_dispatcher_switch_to_view(app->view_dispatcher, G4MEOVERAppViewVarItemList);
}

bool g4meover_app_scene_protocols_freqs_static_on_event(void* context, SceneManagerEvent event) {
    G4MEOVERApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(
            app->scene_manager, G4MEOVERAppSceneProtocolsFreqsStatic, event.event);
        consumed = true;
        switch(event.event) {
        ...
        case VarItemListIndexAddStaticFreq:
            scene_manager_set_scene_state(
                app->scene_manager, G4MEOVERAppSceneProtocolsFreqsAdd, false);
...
void g4meover_app_scene_protocols_freqs_static_on_exit(void* context) {
    G4MEOVERApp* app = context;
